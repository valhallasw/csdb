...
namespace CodeRun.Utils.CommandLine
{
	...
	{
		internal CommandLineArgumentsParser(object target)
		{
			Target = target;
			...
			foreach (string arg in args)
			{
				...
				else if (arg.StartsWith("/"))
				{
					...
					if (arg2.Contains(":"))
					{
						...
						var name = tokens[0];
						var value = tokens[1];
						var pi = FindTargetProperty(name, new Type[] { typeof(bool), typeof(bool?) });
						if (pi == null)
		...
		{
			var t = new T();
			var parser = new CommandLineArgumentsParser(t);
			parser.Parse(args);
			return t;
		}
		public static void ParseInto(object obj, string[] args)
		{
			var parser = new CommandLineArgumentsParser(obj);
			parser.Parse(args);
		}
		public static void ParseInto(object obj, string args)
		{
	...
	public class CommandLineArgumentsGenerator
	{
